...
namespace TestConsole
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var models = new ModelBuilder<Student>(config.WithParent(s => s.GroupId, s => s.Group, g => g.Id, group)).Build(1);
            var models2 = new ModelBuilder<Student>(config.WithParent(s => s.GroupId, s => s.Group, g => g.Id, group2)).Build(1);
